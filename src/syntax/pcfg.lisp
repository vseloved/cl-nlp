...
(defmethod parse-n ((grammar pcfg) (sentence list) n)
  "Return a list of N best parse tree of SENTENCE for PCFG.
   ...
   The default method returns a single tree for the ROOT slot of PCFG."
  (CKY (let* ((cur (cons rule (1- s)))
              (l (atomize (@ pi0 (1- i) (1- s) (second rule))))
              (r (atomize (@ pi0 s (1- j) (third rule))))
              (score (when (and l r)
                       (+ (log q) l r)))
              prev-s prev-r)
         (when score
  ...
  (with-raw-results
    ...
    (when root-rules
      (ds-bind (rule . prob)
          (reduce #`(if (> (cdr %) (cdr %%)) % %%)
                  (mapcar #`(cons (car %)
                                  (if-it (@ pi0 0 last (nt->idx (cadar %)))
                                         (* it (cdr %))
                                         min))
                          root-rules))
        (setf (@ pi0 0 last iroot) prob
              (@ bps 0 last iroot) (cons (mapcar #'nt->idx rule)
                                         last))))
  ...
  (with-raw-results
    ...
    (when root-rules
      (loop :for (rule . prob) :in root-rules
         :collect (cons (mapcar #'nt->idx rule) last) :into rules
         :collect prob :into probs
         :finally (setf (@ pi0 0 last iroot) probs
                        (@ bps 0 last iroot) rules)))
